...
func (c *Cacher) Flush(db int) error {
	_, err := c.Do("select", fmt.Sprintf("%d", db))
	fmt.Println(err)
	if err != nil {
		return err
	}
...
// 获取redis信息
func (c *Cacher) GetAllDb() ([]DB, error) {
	do, err := c.Do("info", "keyspace")
	if err != nil {
		return nil, err
	}
	result := fmt.Sprintf("%s", do)
	return c.RedisInfoToRedisStruct(result), err
}

func (c *Cacher) RedisInfoToRedisStruct(info string) (dbList []DB) {
	...
	for _, s := range split[1:] {
		...
		i := strings.Split(strings.TrimSpace(s), ",")[0]
		r := strings.Split(strings.TrimSpace(i), ":")
		db.Id,_ = strconv.Atoi(strings.Split(r[0], "db")[1])
		db.Name = r[0]
		db.Keys, _ = strconv.Atoi(strings.Split(r[1], "=")[1])
